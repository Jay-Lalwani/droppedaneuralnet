...
def load_data_csv(data_file: str, device: torch.device, target_col: str = "pred"):
    ...
    with open(data_file, "r") as f:
        ...
        # Expect measurement_0..measurement_47 and target col
        for row in reader:
...
def blocks_to_piece_perm(block_order: List[int],
...
# -----------------------------
# Stage 1: Pair up/down into blocks (assignment)
# -----------------------------
@torch.no_grad()
def pairing_cost_matrix(
    ...
    """
    Returns cost matrix C [n_up, n_down], lower is better pairing.
    Cost is based on:
      - mean squared update magnitude ||down(ReLU(up(x)))||^2
      - penalty for extreme activation rate in up pre-activation
      - penalty for large mean output (bias-y blowups)
    """
    n_up = len(up_idxs)
    n_dn = len(down_idxs)
    ...
    for i, ui in enumerate(up_idxs):
        ...
        act = (pre > 0).float().mean().item()                             # scalar
        act_pen = abs(act - 0.5)

        h = F.relu(pre)                                                   # [B,96]
        # out_all[j] = h @ W_dn[j].T + b_dn[j]
...
def solve_assignment(cost: torch.Tensor) -> List[int]:
    """
    Returns matching: for each row i, matched column match[i].
    Uses scipy Hungarian if available, else greedy.
    """
    ...
    try:
        ...
        logging.warning(f"Hungarian unavailable (scipy missing?) or failed: {e}. Falling back to greedy.")
        n = cost_cpu.shape[0]
        used = set()
        match = [-1] * n
        for i in range(n):
...
def stage1_pair_blocks(
    ...
    for i, j in enumerate(match):
        block_pairs.append((up_idxs[i], down_idxs[j]))
    return block_pairs

# -----------------------------
# Stage 2: Build ordering via transition costs (greedy + 2opt)
# -----------------------------
@torch.no_grad()
def block_health_costs(
    ...
    """
    Compute a 'health' cost for applying each block to x.
    Vectorized over blocks:
      W_up: [B,96,48], b_up:[B,96]
      W_dn: [B,48,96], b_dn:[B,48]
    Returns costs: [B]
    """
    # pre: [B, N, 96]
    pre = torch.einsum("nd,bod->bno", x, W_up) + b_up[:, None, :]  # x:[N,48]
...
def transition_cost_matrix(
    ...
    Returns:
      ...
      w[i,j]: cost of using block j after applying block i to x_seed (1-step lookahead)
    """
    B = len(block_pairs)
    ...
    for i in range(B):
        ...
        w[i] = block_health_costs(x_i, W_up, b_up, W_dn, b_dn)                 # [B]
...
def greedy_path(start_cost: torch.Tensor, w: torch.Tensor) -> List[int]:
    ...
    while unused:
        prev = order[-1]
        # pick next with minimal transition cost
...
def two_opt_improve(order: List[int], start_cost: torch.Tensor, w: torch.Tensor, max_passes: int = 3) -> List[int]:
    """
    2-opt on a path (not cycle). Uses edge-cost surrogate.
    """
    best = order[:]
...
# -----------------------------
# Stage 3: Simulated annealing on true MSE
# -----------------------------
...
def stage3_refine_sa(
    ...
    logging.info("Stage 3: simulated annealing refinement on true MSE (subsampled + periodic full eval)")
    ...
    # Evaluate full for baseline
    best_full = mse_for_block_order(pieces, block_pairs, best, last_idx, x_full, y_full)

    logging.info(f"  SA init: sub_mse={best_sub:.10f} full_mse={best_full:.10f}")
    ...
    for t in range(1, iters + 1):
        ...
        # Track best-on-subsample; occasionally verify on full
        ...
        if t % 200 == 0:
            ...
            last_log = now
            logging.info(
                f"  SA t={t:5d}/{iters} T={T:.2e} curr_sub={curr_loss:.10f} "
                f"best_sub={best_sub:.10f} best_full~={best_full:.10f} ({dt:.2f}s)"
            )

            # early stop if essentially zero
            if best_full <= 1e-15:
...
def main():
    ...
    ap.add_argument("--sa_sub_batch", type=int, default=2048)
    ap.add_argument("--sa_iters", type=int, default=8000)
    ap.add_argument("--sa_T0", type=float, default=1e-2)
    ap.add_argument("--sa_Tmin", type=float, default=1e-5)
    ap.add_argument("--eval_full_every", type=int, default=200)
    args = ap.parse_args()
    ...
    # Minibatches for different stages
    n = x_full.shape[0]
    perm = torch.randperm(n, device=device)
    x_pair = x_full[perm[:min(args.pair_batch, n)]]
    x_order = x_full[perm[:min(args.order_batch, n)]]

    # SA subsample (different draw)
    perm2 = torch.randperm(n, device=device)
    x_sub = x_full[perm2[:min(args.sa_sub_batch, n)]]
    y_sub = y_full[perm2[:min(args.sa_sub_batch, n)]]

    # ----------------- Stage 1 -----------------
    t0 = time.time()
    block_pairs = stage1_pair_blocks(pieces, up_idxs, down_idxs, x_pair)
    # naive initial order: as paired
    init_order = list(range(len(block_pairs)))
    init_piece_perm = blocks_to_piece_perm(init_order, block_pairs, last_idx)
    mse1 = mse_for_perm(pieces, init_piece_perm, x_full, y_full)
    logging.info(f"Stage 1 done in {time.time()-t0:.2f}s | full MSE (paired, naive order) = {mse1:.10f}")

    # ----------------- Stage 2 -----------------
    t0 = time.time()
    order2 = stage2_order_blocks(pieces, block_pairs, x_order)
    perm2_piece = blocks_to_piece_perm(order2, block_pairs, last_idx)
    mse2 = mse_for_perm(pieces, perm2_piece, x_full, y_full)
    logging.info(f"Stage 2 done in {time.time()-t0:.2f}s | full MSE (surrogate order) = {mse2:.10f}")

    # ----------------- Stage 3 -----------------
    t0 = time.time()
    best_order = stage3_refine_sa(
        pieces=pieces,
    ...
    best_piece_perm = blocks_to_piece_perm(best_order, block_pairs, last_idx)
    mse3 = mse_for_perm(pieces, best_piece_perm, x_full, y_full)
    logging.info(f"Stage 3 done in {time.time()-t0:.2f}s | full MSE (refined) = {mse3:.12f}")

    # Output final permutation (piece indices in application order)
    print("\nBEST PIECE PERMUTATION (length={}):".format(len(best_piece_perm)))
    print(best_piece_perm)
    print("\nBest full MSE:", mse3)
